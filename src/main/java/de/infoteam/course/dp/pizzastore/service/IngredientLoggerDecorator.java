...
public class IngredientLoggerDecorator implements ShoppingListLogger<Ingredient> {
	...
	private final Map<String, Integer> consumedDough = new HashMap<>();
	private final Map<String, Integer> consumedSauce = new HashMap<>();
	private final Map<String, Integer> consumedCheese = new HashMap<>();
	private final Map<String, Integer> consumedTopping = new HashMap<>();
	...
	private void outputAggregation(Map<String, Integer> consumed, PrintStream ps) {
		consumed.forEach((key, value) -> ps.println("\t" + value + "x\t" + key));
	}
	...
	private void aggregateIngredientsInMaps() {
		...
		logger.getConsumedIngredients().stream().filter(Topping.class::isInstance).map(Ingredient::name)
				.forEach(name -> this.consumedTopping.merge(name, 1, Integer::sum));
	}
